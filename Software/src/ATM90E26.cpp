...
void initATM90E26()
{
	...
	mmode0_val |= (atm90e26_lgain_register[setting_current_pga_gain]) << 13;	// Lgain
	...
	write_register(PLconstH, (((uint32_t)setting_pl_const_24) >> 16) & 0xFFFF);
	write_register(PLconstL, ((uint32_t)setting_pl_const_24) & 0xFFFF);
	write_register(MMode,    mmode0_val);

	write_register(AdjStart,0x5678);
	write_register(Ugain,   (uint16_t)setting_voltage_gain);
	write_register(IgainL,  (uint16_t)setting_current_gain);
	write_register(Uoffset, (uint16_t)setting_voltage_offset);
	write_register(IoffsetL,(uint16_t)setting_current_offset);
	write_register(PoffsetL,(uint16_t)setting_act_power_offset);
	write_register(QoffsetL,(uint16_t)setting_rct_power_offset);

	pga_correction_factor = (1.0 + setting_pga_gain_error[setting_current_pga_gain] / 10000.0) / atm90e26_lgain_factor[setting_current_pga_gain];
}
